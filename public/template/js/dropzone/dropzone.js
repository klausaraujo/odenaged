	...
	$('#drop-files').bind('drop', function(e) {
		...
		$.each(files, function(index, file) {
			...
			if (!files[index].type.match('image.*')) {
				...
				//if(errMessage == 0) {
					$('#drop-files').html('Solo se pueden cargar imagenes');
					++errMessage
				/*}
			...
			var fileReader = new FileReader();
				...
				fileReader.onload = (function(file) {
					...
					return function(e) {
						...
						/*let del = '<a href="#" class="delete" style="margin-top:30px">Eliminar</a><br><br><br>';
						let input = '<input type="text" class="form-control" style="background-color:#fff" placeholder="Descripci&oacute;n" />'*/
						...
						/*if($('#dropped-files > .image').length < maxFiles) { 
							...
							$('#dropped-files').append('<div id="image'+i+'" class="col-sm-3 image" style="background: url('+image+');'+
														'background-size:cover;padding:3px">'+del+input+'</div>');
							...
							$('#extra-files #file-list ul').append('<li><div id="image'+i+'" class= col-sm-3 "image" style="background:'+
														'url('+image+'); background-size:cover;padding: 3px">'+del+input+'</div></li>');
